...
# This python file counts the number of seconds the solution took and outputs it to an excel file
...
for filename in filenames:
    ...
    for method_string in methods:
        ...
        if solution_node:
            print(f"{method_string.capitalize()} search on {filename} completed in {execution_time:.2f} seconds.")
            row_data[method_string] = f"{execution_time:.2f}"
        else:
            print(f"{method_string.capitalize()} search on {filename} found no solution.")
            row_data[method_string] = "No solution"
